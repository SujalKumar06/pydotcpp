...
enum class ASTStmtNodeType {
  ...
  IF_STMT,    // if statement. Points to condition (BINARY or UNARY or other
           // relevant nodes) and BLOCK
...
public:
  VarDeclNode(std::unique_ptr<ASTExprNode> name,
              std::unique_ptr<ASTExprNode> value);

  std::unique_ptr<ASTExprNode>
      name; // A reference node which stores the name of the variable
  std::unique_ptr<ASTExprNode> value; // A number, string or boolean node that
                                      // stores the value inside the variable
};
...
public:
  IfStmtNode(std::unique_ptr<ASTExprNode> condition,
...
public:
  ElifStmtNode(std::unique_ptr<ASTExprNode> condition,
               std::unique_ptr<ASTStmtNode> block);
...
public:
  WhileStmtNode(std::unique_ptr<ASTExprNode> condition,
                std::unique_ptr<ASTStmtNode> block);
...
public:
  ForStmtNode(std::unique_ptr<ASTExprNode> condition,
              std::unique_ptr<ASTStmtNode> block);
